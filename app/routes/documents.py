...
def chat_index():
    ...
    existing_files = [
        ...
        if os.path.exists(os.path.join(session["CHAT_UPLOAD_DIR"], f))
    ]
...
def upload_file():
    ...
    for file in uploaded_files:
        ...
        if (
            not file
            or file.content_length > MAX_FILE_SIZE
            or not allowed_file(secure_file_name)
        ):
            response["messages"].append(
                f"File {secure_file_name} is either too large or not of an allowed type."
            )
            continue
        if file and allowed_file(secure_file_name):
            ...
            if filename.endswith(".pdf") or filename.endswith(".docx"):
                ...
                if txt_filepath:
                    ...
                    response["messages"].append(
                        f"File {secure_file_name} conversion to .txt failed."
                    )
                    continue

            new_files.append(os.path.basename(filename))
            response["messages"].append(
                f"File {os.path.basename(filename)} processed successfully."
            )
    ...
    try:
        ...
        response["status"] = "error"
        response["messages"].append(
            f"Error generating embeddings for {secure_file_name}: {str(e)}"
        )
...
def remove_file():
    ...
    return jsonify(
        {"status": "success", "message": f"File {file_name} deleted successfully"}
    )
