...
from app.utils.assets_util import compile_maintenance_assets
...
def create_app():
    ...
    assets = Environment(app)  # Initialize the assets environment
    CSRFProtect(app)
    CORS(app)
    if maintenance_mode:
        from app.modules.maintenance import maintenance

        app.register_blueprint(maintenance.maintenance_bp)

        @app.route("/")
        def root():
            return redirect(url_for("maintenance_bp.maintenance"))

        # Compile static assets for maintenance
        compile_maintenance_assets(assets)
    else:
