...
bcrypt = Bcrypt()
mail = Mail()
socketio = SocketIO(message_queue="amqp://", async_mode="eventlet", engineio_logger=True, logger=True)
...
def create_app():
    ...
    socketio.init_app(app, message_queue=app.config["CELERY_BROKER_URL"], async_mode="eventlet", engineio_logger=True, logger=True)
